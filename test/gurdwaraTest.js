...
describe('Gurdwaras', () => {
	...
	describe('/POST Gurdwara', () => {
		it('Should not POST a Gurdwara without address field', (done) => {
			let gurdwara = {
				"name": "Test Gurdwara",
				"city": "Test City",
				"state": "Test State",
		...
		it('Should Post a Gurdwara', (done) => {
			...
			chai.request(server)
				...
				.end((err, res) => {
					res.should.have.status(200);
					res.body.should.be.a('object');
					res.body.should.have.property('message').eql('Gurdwara successfully added!');
			...
			gurdwara.save((err, gurdwara) => {
				chai.request(server) 
					...
					.end((err, res) => {
						res.should.have.status(200);
						res.body.should.be.a('object');
						res.body.should.have.property('message').eql('Gurdwara updated!');
						res.body.gurdwara.should.have.property('country').eql('USA');
					done();
				});
			});
		});
		...
		it('Should DELETE a Gurdwara given the id', (done) => {
			...
			gurdwara.save((err, gurdwara) => {
				chai.request(server)
					...
					.end((err, res) => {
						res.should.have.status(200);
						res.body.should.have.property('message').eql('Gurdwara successfully deleted!');
					done();
				});	
			});
		});
